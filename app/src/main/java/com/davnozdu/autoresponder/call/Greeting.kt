...
object Greeting {
    ...
    // Параметры бипа — короткий ненавязчивый сигнал после приветствия.
    private const val SR = 48000
    private const val BEEP_GAP_MS = 1200  // пауза перед бипом (была 400, затем 800 — пользователь попросил ещё больше)
    private const val BEEP_MS = 400       // была 1300 (долгий гудок) — пользователь снова пожаловался «противный»,
                                           // короткий классический сигнал вместо долгого тона
    private const val BEEP_HZ = 1000.0
    private const val BEEP_AMP = 0.18     // была 0.6, затем 0.35 — оба раза «противно орёт», снижена ещё раз
