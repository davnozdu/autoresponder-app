...
object Responder {
    ...
    private fun historyBlock(context: Context, key: String?, incomingText: String? = null,
        ...
        return try {
            ...
            // и обе стороны переписки мессенджеров из копий, которые кладёт модуль.
            //
            // Здесь ЖДЁМ модуль, хотя клиент ждёт ответа. Раньше ждать было незачем — копии
            // обновлял таймер каждые десять минут, — но таймер убран: он переписывал сотни
            // мегабайт в сутки впустую. Теперь единственный момент, когда свежесть переписки
            // действительно нужна, — вот этот, и бюджет ожидания короткий: копии лежат в ОЗУ,
            // обновляется только изменившееся. Не успел — отвечаем по тому, что есть.
            SmsWatcher.drain(context)
            MsgrBridge.sync(context, force = true, waitMs = MsgrBridge.replyWaitMs())
