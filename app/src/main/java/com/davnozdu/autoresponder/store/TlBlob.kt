 ...
 *
 * Дата может встретиться в блобе и раньше — у пересланного сообщения своя дата внутри
 * `fwd_from`. Поэтому берём ПОСЛЕДНЕЕ вхождение, за которым лежит корректная строка:
 * собственная дата сообщения всегда правее вложенных.
 *
 * Проверено на устройстве по всей базе: 25 534 записи, 0 сбоев разбора, 18 131 текст,
 * 6 668 без текста (медиа без подписи), 735 служебных.
 */
object TlBlob {
    ...
    fun messageText(blob: ByteArray, date: Int): String? {
        ...
        while (true) {
            ...
            if (at < 0) break
            readString(blob, at + 4)?.let { best = it.first }
            from = at + 1
        }
