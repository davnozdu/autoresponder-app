    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            ...
            com.davnozdu.autoresponder.store.Heartbeat.ACTION -> {
                ...
                // Тот же будильник ведёт журнал: подбираем исходящие, которые робот не
                // отправлял (SMS владельца, переписка в мессенджерах). Отдельного будильника
                // заводить незачем, а этот переживает Doze и перезагрузку.
                //
                // Будильник тикает раз в десять минут, но мост это НЕ значит: он сам
                // пропускает фоновые просьбы чаще раза в час (MsgrBridge.MIN_GAP_MS).
                // Чтение SMS-провайдера дёшево и идёт как раньше, а копирование баз —
                // нет. Работа с БД и файлами — не на потоке ресивера.
