...
if (transferred) {
    ...
    // Главная запись для этого сценария — ВСЕГДА своя (pal_record, перезапущенная в момент
    // переброса): именно она содержит чистое сообщение клиента, а не весь звонок с ожиданием.
    // OEM-запись (если найдётся) тоже сохраняем — она писала непрерывно с начала звонка,
    // отдельной, НЕ главной записью в том же списке, а не отбрасываем.
    ...
    // Секундная (не главная) запись — OEM, если штатный рекордер поймал звонок с самого
    // начала (ожидание+сообщение одним файлом). Ищем ПОСЛЕ того, как своя уже сохранена,
    // чтобы не блокировать ею основной путь, если OEM не найдётся или найдётся не сразу.
...
В `AmRecordingsActivity` запись с `reason="voicemail"` — главная, показывается
как основная строка; `"voicemail_full"` (если есть) — рядом/под ней как
дополнительная («полная запись звонка»), UI-деталь на усмотрение реализации.
...
- Поле «Ждать решения, мин» и «Длительность записи после переброса, мин» —
  в UI ввод и отображение в МИНУТАХ (пользователь явно попросил), хранение в
  `Settings` — по-прежнему в секундах (`screeningWaitSec`/`voicemailMaxSec`,
  как у остальных `*Sec`-полей); конвертация на границе поля, как уже сделано
  для `batchWait` (секунды на экране, мс в `Settings.batchWaitMs`). Дефолты:
  5 мин (300с) и 1 мин (60с).
...
4. **Своя запись (передёрнутая в момент переброса) — всегда главная**, даже
   если OEM тоже что-то записал: OEM пишет непрерывно с начала звонка и не
   даёт чистого среза «только сообщение». OEM-запись НЕ отбрасывается — если
   найдётся, сохраняется отдельной, НЕ главной записью в том же списке.
5. **`screeningWaitSec`/`voicemailMaxSec` настраиваемые пользователем**,
   отображаются в UI в минутах (хранятся в секундах), дефолты 5 мин / 1 мин.
